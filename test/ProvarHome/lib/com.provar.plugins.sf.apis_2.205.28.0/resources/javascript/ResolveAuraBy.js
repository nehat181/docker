...
function resolveAuraBy(componentXPath, searchContext, multiple) {
    ...
    var flexiXpath="//div[((contains(@class,'windowViewMode-normal') and contains(@class,'oneContent')) or (contains(@class,'windowViewMode-maximized'))) and contains(@class,'active') and contains(@class,'lafPageHost')]//one-record-home-flexipage2//flexipage-component2[@data-component-id]";
    ...
    if(flexiNodes.length > 0){
        for(let i=0;i<flexiNodes.length;i++){
            ...
            var dataAuraNode=xpathResultNode.iterateNext();
            let dataAuraBy = null;
            while(dataAuraNode){
                dataAuraBy = dataAuraNode ? dataAuraNode.getAttribute('data-aura-rendered-by') : null;
                if(dataAuraBy){
    ...
    if(dataAuraRenderedBy.size > 0){
    	dataAuraRenderedBy.forEach(value => {
    		if(!auraInfo2[value]){
				const ai = extractAuraInfo(false,null,value);
				if(ai) {
    ...
    if(resultElem && qualifier && qualifier.length === 2){
        ...
        while (resultElem) {
			...
			if(htmlElemTagName !== "LIGHTNING-LOOKUP"){
				...
				var qualifierText=lookupAuraInfo[lookupAuraInfo.expressionKey];
                ...
                if(qualifier[1] === "'"+qualifierText+"'"){
	                	elemsIt3=document.evaluate(".//input[contains(@class,'slds-combobox__input')]", htmlElem, nsResolver, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	                	if(elemsIt3){
	                		return elemsIt3;
	                	}
                }
